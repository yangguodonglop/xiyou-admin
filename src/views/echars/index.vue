    ...
    <el-row>
        <el-col :span="12">
            <div class="div_item" style="width:100%;height:50px;">
                <div class="item_start">
                    ...
                    <el-date-picker v-model="valueOnlinestrat" type="datetime" placeholder="选择日期时间" @change="getSTimeOnlinestrat" value-format="yyyy-MM-dd HH:mm">

                    </el-date-picker>
                </div>
                <div class="item_end" style="">
                    <span class="demonstration">结束时间:</span>
                    <el-date-picker v-model="valueOnlineend" type="datetime" placeholder="选择日期" @change="getSTimeOnlineend" value-format="yyyy-MM-dd HH:mm">
                        >
                    </el-date-picker>
                </div>
        ...
        <el-col :span="12">
            <div class="div_item" style="width:100%;height:50px;">
                <div class="item_start">
                    <span class="demonstration">开始时间:</span>
                    <el-date-picker v-model="valueDevStorestart" type="datetime" @change="getSTimeDevStorestart" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期">
                    </el-date-picker>
                </div>
                <div class="item_end" style="">
                    <span class="demonstration">结束时间:</span>
                    <el-date-picker v-model="valueDevStoreend" type="datetime" @change="getSTimeDevStoreend" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期">
                    </el-date-picker>
                </div>
                <div class="item_button">
        ...
        <el-col :span="12">
            <div class="div_item" style="width:100%;height:50px;">
                <div class="item_start">
                    <span class="demonstration">开始时间:</span>
                    <el-date-picker v-model="valueUsagestart" type="datetime" @change="getSTimeUsagestart" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期">
                    </el-date-picker>
                </div>
                <div class="item_end" style="">
                    <span class="demonstration">结束时间:</span>
                    <el-date-picker v-model="valueUsageend" type="datetime" @change="getSTimeUsageend" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期">
                    </el-date-picker>
                </div>
                <div class="item_button">
        ...
        <el-col :span="12">
            <div class="div_item" style="width:100%;height:50px;">
                <div class="item_start">
                    <span class="demonstration">开始时间:</span>
                    <el-date-picker v-model="valueFilestart" type="datetime" @change="getSTimeFilestart" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期">
                    </el-date-picker>
                </div>
                <div class="item_end" style="">
                    <span class="demonstration">结束时间:</span>
                    <el-date-picker v-model="valueFileend" type="datetime" @change="getSTimeFileend" value-format="yyyy-MM-dd HH:mm" placeholder="选择日期">
                    </el-date-picker>
                </div>
                <div class="item_button">
        ...
        querySummary(Summaryparam)
            .then(data => {
                ...
                if (status != 0) {
                    ...
                    //储存量
                    this.datatwo = data.data.TotalStoreUsage;
                    //存储空间
                    this.datatwoactive = data.data.TotalMinerStoreCap;
                    this.datafour = data.data.TotalFileCnt; //实际存储数据量
                    this.datathree = data.data.FileTotalStoreUsage; //实际存储文件个数
                }
            })
            .catch(error => {
                this.$message({
        ...
        queryDevStoreHistgraph(timeParams)
            .then(data => {
                ...
                if (status != 0) {
                    ...
                    if (data.data.ipfsCapList.length >= 0) {
                        for (var i = 0; i < data.data.ipfsCapList.length; i++) {
                            //将字节转化为GB
                            data.data.ipfsCapList[i] = (data.data.ipfsCapList[i] / 1024 / 1024 / 1024).toFixed(1)
                        }
                    }
                    if (data.data.totalDevCapList.length >= 0) {
                        for (var i = 0; i < data.data.totalDevCapList.length; i++) {
                            //将字节转化为TB
                            data.data.totalDevCapList[i] = (data.data.totalDevCapList[i] / 1024 / 1024 / 1024 / 1024).toFixed(1)
                        }
                    }
        ...
        queryStoreUsageHistgraph(timeParams)
            .then(data => {
                ...
                if (status != 0) {
                    ...
                    if (data.data.storeUsage.length >= 0) {
                        for (var i = 0; i < data.data.storeUsage.length; i++) {
                            //将字节转化为GB
                            data.data.storeUsage[i] = (data.data.storeUsage[i] / 1024 / 1024 / 1024).toFixed(1)
                        }
                    }
